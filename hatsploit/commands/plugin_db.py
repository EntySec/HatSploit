...
class ExternalCommand(Command):
    def __init__(self):
        super().__init__({
            ...
            'MinArgs': 1,
            'Options': {
                'list': ['', "List all connected plugin databases."],
                'disconnect': ['<name>', "Disconnect specified plugin database."],
                'connect': ['<name> <path>', "Connect new plugin database."],
                'build': [
                    '<path> <output_path>',
                    "Build plugin database from plugins path."
                ],
            },
        })
    ...
    def run(self, args):
        if args[1] == 'list':
            ...
            self.db.disconnect_plugin_database(args[2])

        elif args[1] == 'build':
            self.builder.build_plugin_database(args[2], args[3])

        elif args[1] == 'connect':
            self.db.connect_plugin_database(args[2], args[3])
