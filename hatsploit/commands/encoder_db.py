...
class ExternalCommand(Command):
    def __init__(self):
        super().__init__({
            ...
            'MinArgs': 1,
            'Options': {
                'list': ['', "List all connected encoder databases."],
                'disconnect': ['<name>', "Disconnect specified encoder database."],
                'connect': ['<name> <path>', "Connect new encoder database."],
                'build': [
                    '<path> <output_path>',
    ...
    def run(self, args):
        if args[1] == 'list':
            ...
            self.db.disconnect_encoder_database(args[2])

        elif args[1] == 'build':
            self.builder.build_encoder_database(args[2], args[3])

        elif args[1] == 'connect':
            self.db.connect_encoder_database(args[2], args[3])
